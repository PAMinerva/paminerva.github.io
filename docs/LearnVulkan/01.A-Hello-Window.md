...
![Image](images/01/vkHelloWindow.png)
...
><br>
>
>![Image](images/01/execution-model.png)
>
><br>
>
...
SPIR-V is a binary intermediate representation (IR) for shader programs. In Vulkan, you can still write the shader code for programmable stages in a high-level shading language such as GLSL or HLSL, but a SPIR-V binary representation is needed when setting the pipeline state. Since it is pre-compiled, the driver can easily convert SPIR-V IR blob to shader machine (GPU) code.
...
![Image](images/01/VK-architecture.png)
...
![Image](images/01/dispatchable-object.png)
...
![Image](images/01/instance_call_chain.png)
...
![Image](images/01/device_call_chain.png)
...
![Image](images/01/device_call_chain_2.png)
...
><br>
>
>![Image](images/01/physical-is-instance.png)
...
![Image](images/01/instance_chain.png)
...
![Image](images/01/device_chain.png)
...
![Image](images/01/win-client-area.png)
...
![Image](images/01/gpu-queues-engines.png)
...
![Image](images/01/vk-queue-families.png)
...
![Image](images/01/swapchain-images.png)
...
>The use of the old swapchain at the beginning of **CreateSwapchain** will be explained in a later tutorial, as it is not relevant for the current discussion.
...
![Image](images/01/immediate-mode.png)
...
![Image](images/01/mailbox-mode.png)
...
><br>
>
>![Image](images/01/compositing-window-manager.png)
>
><br>
>
...
><br>
>
>![Image](images/01/v-interval.png)
>
><br>
>
...
><br>
>
>![Image](images/01/fast-gpu.png)
>
><br>
>
><br>
>
>![Image](images/01/slow-gpu.png)
...
Draw commands must be recorded within a render pass instance on a per-subpass basis. A render pass instance defines the use of a render pass in a command buffer. A render pass represents a collection of attachment descriptors, subpasses, subpass dependencies, and describes how the attachments will be used over the course of the various subpasses that comppose a render pass.

An attachment is an image view used in a framebuffer, which is a collection of image views and a set of dimensions that, in conjunction with a render pass, define the inputs and outputs used by drawing commands recorded in one or more subpasses that compose a render pass.
...
><br>
>
>![Image](images/01/tile-based-rendering.png)
>
><br>
>
...
Here, we create an array of **VkSubpassDependency** with a single element to specify that our subpass (**srcSubpass** = 0) will depend on drawing commands executed outside (**srcSubpass** = **VK_SUBPASS_EXTERNAL** means before) the current render pass instance. In particular, we indicate that the GPU must finish executing specific stages of the pipeline triggered by drawing commands executed before the current render pass instance before executing reading and writing operations in the same stages but triggered by drawing commands of the current render pass instance. **VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT** refers to the pipeline stage where the final color values are written to the color attachment (the render target). Indeed, the dependecy between the current render pass instance and the previous one concerns the concurrent access to the color attachment.
...
![Image](images/01/viewport-scissor.png)
