...
SPIR-V is a binary intermediate representation for shader programs. In Vulkan, you can can still write the shader code for programmable stages in a high-level shading language such as GLSL or HLSL, but a SPIR-V binary code is needed when setting the pipeline state. Since it is pre-compiled, the driver can easily convert it to shader machine (GPU) code.
