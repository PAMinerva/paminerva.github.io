...
Sometimes, performance enhancement is also achievable through asynch compute. This involves recording compute and graphics work asynchronously and queuing corresponding command buffers in different GPU queues, if supported. It's important to note that this approach relies on the availability of multiple queues on the device capable of handling various command types (graphics, compute, etc.), a feature often lacking, especially on integrated and mobile GPUs.
...
- **Synchronize**: Synchronize both the transform feedback and counter buffers.

<br>

In contrast, using a compute shader, we can achieve the same particle effects with a more straightforward approach:
...
- **Synchronization**: Synchronize graphics and compute works for the current storage buffer only.
