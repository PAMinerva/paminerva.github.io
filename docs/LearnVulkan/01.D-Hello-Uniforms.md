...
As we know, shaders require input data from previous stages in order to perform their tasks. For example, in previous tutorials, we demonstrated how vertex data stored in a vertex buffer is passed from one stage to the next for processing. However, programmable stages can also access resources stored in device memory, referred to as shader resources, using descriptor sets and pipeline layouts. We discussed the theory of this in a previous tutorial (refer to **01.B-Hello-Triangle**, section **3.3.1**), and now it's time to put that knowledge into practice. <br>
...
><br>
>
>you may encounter alignment and\or size issues. In this case the **position** member has a base alignment and size of 16 bytes in the shader code, and starts at a byte offset of 0 from the beginning of the structure, which is a multiple of 16. If the memory allocated in device memory for this buffer is correctly aligned, there is no alignment problem with the **position** member. However, there is a size discrepancy between GLSL and C++: in GLSL **position** takes up 16 bytes, while in C++ it takes up 12 bytes. As a result the **color** member starts at a byte offset of 16 in the shader code, while in the C++ code it starts at 12. In other words, the memory layout of the buffer will be interpreted differently by our application and the shader program, as shown in the following listing
>
><br>
>
>```
...
><br>
>
>The reason for this is that the virtual addresses used by the application (CPU) and the device (GPU) are different, but they are mapped to the same physical address, meaning that both the CPU and GPU access the same memory region. Because **memcpy** transfers data as a bit stream, the source data starting at byte offset 12 will be interpreted as the last component of the position member in the shader code.
>
><br>
>
...
> <br>
>
>In this tutorial series, we will opt for the first solution as it is less error-prone. Indeed, this approach minimizes the chance of manual padding or alignment operations for each structure defined in the C++ code, making it easier to use.
>
><br>
>
...
In **PopulateCommandBuffer**, we bind the pipeline object (which includes the pipeline layout information) and the descriptor set (which includes our uniform buffer). This enables the device to access the buffer in device memory through the uniform block in the shader code, as all the necessary information is provided.
