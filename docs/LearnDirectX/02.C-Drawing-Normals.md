...
The output of the geometry shader can be sent to the rasterizer, and up to four buffers in GPU memory as well if the Stream-Output stage is enabled (we will cover the stream output stage in the next tutorial).
