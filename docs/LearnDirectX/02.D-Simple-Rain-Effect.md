...
We will call **StepTimer::Tick** in **OnUpdate** so that we can always get the elapsed time from a frame to another in seconds by calling **StepTimer::GetElapsedSeconds**. Observe that **Tick** takes a callback function as a parameter, which is invoked to execute a task at every "timer tick" (that is, whenever we call **Tick**).
