...
Suppose want to record commands in a command list to both read and write a resource. Even though a GPU can execute commands in parallel, it shouldn't start reading the texture if any write operation is still ongoing, otherwise data races can occur. <br>
