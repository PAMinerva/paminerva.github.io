...
he code for **LoadPipeline** remains unchanged from the previous sample (**D3D12HelloWindow**). However, there are several additions in **LoadAssets**, so we will take the time to explain each one.
