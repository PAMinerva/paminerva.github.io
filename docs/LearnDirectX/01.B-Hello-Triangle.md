...
**Triangle List** indicates a series of triangles that make up a mesh. The three vertices of each triangle must be contiguous in the vertex buffer, and in a specific order (clockwise or anti-clockwise).
...
As you can see in the image above, this allows to have an invariant winding order (clockwise or anti-clockwise) of the vertices of each triangle in the strip.
