...
Constant variables which are updated from the CPU may seem a funny way of defining constant values. However, if you thing about it, constant variables still need to be initialized. So, in this context, by update we mean initialize a constant buffer with a value, which will be constant during the execution of the shader programs that will access it in a pipeline run.
