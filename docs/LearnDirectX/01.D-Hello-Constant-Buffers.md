...
"Constant variables which are updated from the CPU" may seem like an oxymoron, but if you think about it, even constant variables need to be initialized. In this context, "update" refers to initializing a constant buffer with data that will remain constant during the execution of the shader programs that access it.
...
If you remember what we stated in the first tutorial, the window procedure keeps processing **WM_PAINT** messages sent directly to it, or posted to the message queue. That way, we can call both **OnUpdate** and **OnRender** in the **WM_PAINT** message handler to render updated geometries on the screen. So far, we just wasted time calling **OnUpdate** as an empty method. Now, we can use it to update the constant buffer data over time before calling **OnRender**.
